...
attributes_df <- subset(geotable, select = -c(Geometry))
...
# Set export paths for shapefile and layername
...
if (grepl("POLYGON", poly.type, fixed = TRUE)){
  spdf <- SpatialPolygonsDataFrame(wkb.data, data = data.frame(attributes_df))
} else if (grepl("LINE", poly.type, fixed = TRUE)){
  spdf <- SpatialLinessDataFrame(wkb.data, data = data.frame(attributes_df))
} else if (grepl("POINT", poly.type, fixed = TRUE)){
  spdf <- SpatialPointsDataFrame(wkb.data, data = data.frame(attributes_df))
} else {
